...
class TorchEngineMinimal(torch.nn.Module):
    ...
    def step(self, batch: dict[str, torch.Tensor], guide=None, alpha=0.0, mode='init'):
        ...
        with (context()):
            ...
            output = res['outputs'] if 'outputs' in res else None
...
def aux_loss(model, guide, which='all'):
    """
    ...
    if which == 'layers and embeddings':
        ...
        # embeddings
        mod1, mod2 = model.encoder.embedding, guide.encoder.embedding
        for p1, p2 in zip(mod1.parameters(), mod2.parameters()):
            sum = sum + ((p1 - p2) ** 2).sum()
